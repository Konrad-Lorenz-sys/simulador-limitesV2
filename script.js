...
function run(calcOnly=false){
  ...
  if (!scaleFixed) { // Solo autoajusta si la escala no está fijada
      ...
      if (yValues.length > 0) {
          ...
          // 1. Si no hay datos (yValues.length == 0), centrar en (0,0) con rango 10. (Ya manejado abajo en el else)
          // 2. Si el rango es el valor inicial (10), forzar la vista en (0,0).
          if (currentYRange === 0 || $('#range').value === '5') { 
              dragYCenter = 0; 
              currentYRange = 10; // Rango inicial Y
          } else if (est.type === 'finite') {
              // Si ya se ha movido o hecho zoom, y hay límite finito, centrar en L.
              dragYCenter = est.value;
              currentYRange = Math.max(1, currentRange * buffer); 
          } else if (est.type === 'infinite' || currentRange > 20) {
              // Si el límite es infinito, centrar en los datos existentes
